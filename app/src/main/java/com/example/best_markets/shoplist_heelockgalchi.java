...
public class shoplist_heelockgalchi extends AppCompatActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_shoplist_heelockgalchi);

        RatingBar ratingBar = findViewById(R.id.ratingBar1);
        ratingBar.bringToFront();

        // 프래그먼트 객체 선언
        Fragment fragment_menu_heelockgalchi= new MenuFragment_heelockgalchi();
        Fragment fragment_inform_heelockgalchi = new InformationFragment_heelockgalchi();
        ...
        bottomNavigationView.setOnNavigationItemSelectedListener(new BottomNavigationView.OnNavigationItemSelectedListener() {
            ...
            public boolean onNavigationItemSelected(@NonNull MenuItem item) {
                ...
                switch (item.getItemId()) {
                    case R.id.fragment_menu_heelockgalchi:
                        // replace(프레그먼트를 띄워줄 frameLayout, 교체할 fragment 객체)
                        getSupportFragmentManager().beginTransaction().replace(R.id.frame_layout, fragment_menu).commitAllowingStateLoss();
                        return true;
                    case R.id.fragment_information_heelockgalchi:
